...
but they need your help finding the errors. Every item type is identified by a single lowercase or uppercase 
...
group. For efficiency, within each group of three Elves, the badge is the only item type carried by 
all three Elves. That is, if a group's badge is item type B, then all three Elves will have item 
...
efforts: here, they are 18 (r) for the first group and 52 (Z) for the second group. The sum of these is 70.
...
def get_second_solution(i):
    shared_item = list(i[0].intersection(*i[1:]))[0]
...
def convert_to_tuples(f):
    tuples = zip(f[::3], f[1::3], f[2::3])

    three_tuples = list(tuples)

    return three_tuples


clean = lambda a: set(a.strip())
