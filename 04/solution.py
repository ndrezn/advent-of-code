...
def contains(i, j, check_any):
    def cleaner(k):
        vals = [int(i) for i in k.split("-")]
        vals[1] += 1
        return set(range(*vals))

    i, j = cleaner(i), cleaner(j)

    if check_any:
        return bool(i & j)

    return i.issubset(j) or j.issubset(i)


def get_sol(f, check_any):
    return sum(
        map(
            lambda i: contains(*i.strip().split(","), check_any),
            open(f, "r").readlines(),
        )
    )


test_sol = get_sol("04/example.txt", False)

assert test_sol == 2

sol = get_sol("04/input.txt", False)

print(f"Answer part 1: {sol}")
...
test_sol = get_sol("04/example.txt", True)

assert test_sol == 4

sol = get_sol("04/input.txt", True)

print(f"Answer part 2: {sol}")
