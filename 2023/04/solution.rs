...
fn main() {
    ...
    assert_eq!(sol, 13, "Example result for part 1 should be 13, found {:?}", sol);

    let part_one_solution = get_input("input.txt").into_iter();
    ...
    let part_two_example = get_input("example.txt").into_iter();
    let vectors = part_two_example
        .map(|i| generate_vectors(&i, 7));
    let wins = vectors.clone()
        .map(|i| count_winners(&i));
    
    let total_ex_cards = breed_cards(wins);
    // Validate we correctly answered the example
    assert_eq!(total_ex_cards, 30, "Example result for part 2 should be 30, found {:?}", sol);


    let part_two_example = get_input("input.txt").into_iter();
    let vectors = part_two_example
        .map(|i| generate_vectors(&i, 10));
    let wins = vectors.clone()
        .map(|i| count_winners(&i));
